...
(async function() {
    ...
    const faviconOrig = document.querySelectorAll('link[rel="shortcut icon"]')[0];
    ...
    const mainObserver = new MutationObserver(async (mutations) => {
      if (mutations.filter((mutation) => mutation.addedNodes.length > 0 && [...mutation.addedNodes].filter((addedNode) => addedNode.nodeName.toLowerCase() === 'link' && addedNode.getAttribute('rel') === 'shortcut icon').length > 0).length > 0) return;
